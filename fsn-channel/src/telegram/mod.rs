// fsn-channel/src/telegram/mod.rs — Telegram adapter (feature: telegram).
...
///
/// ```rust,ignore
/// use fsn_channel::TelegramAdapter;
///
...
impl TelegramAdapter {
    ...
    pub fn new(config: TelegramConfig) -> Self {
        Self { config, bot: tokio::sync::OnceCell::new() }
    }

    fn bot(&self) -> Result<&Bot, ChannelError> {
        self.bot.get().ok_or_else(|| ChannelError::connection("not connected — call connect() first"))
    }
}
...
impl Channel for TelegramAdapter {
    ...
    async fn connect(&self) -> Result<(), ChannelError> {
        ...
        self.bot.set(bot).map_err(|_| ChannelError::internal("bot already set"))?;
        Ok(())
    }
