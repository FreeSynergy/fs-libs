// fsn-auth/src/oauth2.rs — OAuth2 Authorization Code Flow (feature: oauth2).
...
///
/// ```rust,ignore
/// use fsn_auth::oauth2::{OAuth2Client, OAuth2Config};
///
...
impl OAuth2Client {
    ...
    pub async fn exchange_code(
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| FsnError::network(e.to_string()))?;

        if !resp.status().is_success() {
            let body = resp.text().await.unwrap_or_default();
            return Err(FsnError::auth(format!("token exchange failed: {body}")));
        }

        resp.json::<TokenResponse>()
            .await
            .map_err(|e| FsnError::parse(e.to_string()))
    }

    /// Use a refresh token to obtain a new access token.
    pub async fn refresh_token(&self, refresh_token: &str) -> Result<TokenResponse, FsnError> {
        let params = [
        ...
        if !resp.status().is_success() {
            let body = resp.text().await.unwrap_or_default();
            return Err(FsnError::auth(format!("token refresh failed: {body}")));
        }
