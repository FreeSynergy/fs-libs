// fsn-auth/src/jwt.rs — JWT signing and validation (feature = "jwt")
...
impl JwtSigner {
    ...
    pub fn rsa_pem(
        ...
        let key = EncodingKey::from_rsa_pem(pem)
            .map_err(|e| FsnError::auth(format!("invalid RSA private key PEM: {e}")))?;
        Ok(Self {
            key,
    ...
    pub fn sign(&self, claims: &Claims) -> Result<String, FsnError> {
        ...
        encode(&header, claims, &self.key)
            .map_err(|e| FsnError::auth(format!("JWT signing failed: {e}")))
    }
}
...
impl JwtValidator {
    ...
    pub fn rsa_pem(
        ...
        let key = DecodingKey::from_rsa_pem(pem)
            .map_err(|e| FsnError::auth(format!("invalid RSA public key PEM: {e}")))?;
        Ok(Self {
            key,
            algorithm: JwtAlgorithm::RS256,
            issuer: issuer.into(),
    ...
    pub fn validate(&self, token: &str) -> Result<Claims, FsnError> {
        ...
        decode::<Claims>(token, &self.key, &validation)
            ...
            .map_err(|e| FsnError::auth(format!("JWT validation failed: {e}")))
    }
}
