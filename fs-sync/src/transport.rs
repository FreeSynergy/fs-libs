...
impl WsTransport {
    ...
    pub async fn send(&mut self, bytes: Vec<u8>) -> Result<(), FsError> {
        self.ws
            .send(Message::Binary(bytes.into()))
            .await
            .map_err(|e| FsError::network(format!("WsTransport send failed: {e}")))
    }

    /// Receive the next binary frame. Returns `None` on clean close.
    pub async fn recv(&mut self) -> Result<Option<Vec<u8>>, FsError> {
        loop {
            match self.ws.next().await {
                ...
                Some(Err(e)) => {
                    return Err(FsError::network(format!("WsTransport recv failed: {e}")))
                }
            }
        }
    }
...
///
/// ```rust,ignore
/// use fs_sync::{SyncEngine, SyncSession, WsTransport};
///
...
impl<T: Serialize + for<'de> Deserialize<'de>> SyncSession<T> {
    ...
    pub async fn run_until_synced(&mut self) -> Result<(), FsError> {
        loop {
    ...
    pub async fn push(&mut self) -> Result<bool, FsError> {
    ...
    pub async fn pull(&mut self) -> Result<bool, FsError> {
        match self.transport.recv().await? {
            Some(bytes) => {
                self.doc.receive_sync_message(&mut self.peer, &bytes)?;
                Ok(true)
