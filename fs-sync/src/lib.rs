// fs-sync — CRDT sync layer for FreeSynergy.
//
...
impl<T: Serialize + for<'de> Deserialize<'de>> SyncDoc<T> {
    ...
    pub fn with_value(actor: ActorId, value: &T) -> Result<Self, FsError> {
    ...
    pub fn get(&self) -> Result<T, FsError> {
        ...
        let json = match item.0 {
            automerge::Value::Scalar(s) => match s.as_ref() {
                ...
                other => return Err(FsError::Parse(format!("unexpected scalar: {other:?}"))),
            },
    ...
    pub fn set(&mut self, value: &T) -> Result<(), FsError> {
        let json = serde_json::to_string(value)
            .map_err(|e| FsError::Parse(format!("sync serialize: {e}")))?;
        self.doc
    ...
    pub fn receive_sync_message(
        ...
        self.doc
            ...
            .map_err(|e| FsError::internal(format!("sync apply: {e}")))
    }
