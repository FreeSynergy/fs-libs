...
impl CaBundle {
    ...
    pub fn generate(common_name: &str, days_valid: u32) -> Result<Self, FsnError> {
        let key_pair = KeyPair::generate()
            .map_err(|e| FsnError::internal(format!("CA key generation failed: {e}")))?;
        ...
        let cert = params
            ...
            .map_err(|e| FsnError::internal(format!("CA self-sign failed: {e}")))?;
    ...
    pub fn issue_server_cert(
        ...
        san: &[&str],
        days_valid: u32,
    ) -> Result<CertBundle, FsnError> {
        let ca_key_pair = KeyPair::from_pem(&self.key_pem)
            .map_err(|e| FsnError::internal(format!("CA key parse failed: {e}")))?;
        let ca_cert = CertificateParams::from_ca_cert_pem(&self.cert_pem)
            .map_err(|e| FsnError::internal(format!("CA cert parse failed: {e}")))?
            .self_signed(&ca_key_pair)
            .map_err(|e| FsnError::internal(format!("CA cert rebuild failed: {e}")))?;

        let key_pair = KeyPair::generate()
            .map_err(|e| FsnError::internal(format!("server key generation failed: {e}")))?;
        ...
        for dns_name in san {
            params.subject_alt_names.push(SanType::DnsName(
                (*dns_name).try_into().map_err(|e| {
                    FsnError::internal(format!("invalid SAN DNS name '{dns_name}': {e}"))
                })?,
            ));
        }
        ...
        let cert = params
            ...
            .map_err(|e| FsnError::internal(format!("server cert signing failed: {e}")))?;
    ...
    pub fn issue_client_cert(
        ...
        let key_pair = KeyPair::generate()
            .map_err(|e| FsnError::internal(format!("client key generation failed: {e}")))?;
        ...
        let cert = params
            .signed_by(&key_pair, &ca_cert, &ca_key_pair)
            .map_err(|e| FsnError::internal(format!("client cert signing failed: {e}")))?;
