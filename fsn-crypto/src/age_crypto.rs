...
impl AgeEncryptor {
    ...
    pub fn from_public_key(key: &str) -> Result<Self, FsnError> {
        let recipient = key
            .parse::<age::x25519::Recipient>()
            .map_err(|e| FsnError::config(format!("invalid age public key: {e}")))?;
        Ok(Self { recipient })
    }

    /// Encrypt plaintext. Returns armored ASCII output.
    pub fn encrypt(&self, plaintext: &[u8]) -> Result<Vec<u8>, FsnError> {
        let recipient = self.recipient.clone();
        let encryptor =
            age::Encryptor::with_recipients(std::iter::once(&recipient as &dyn age::Recipient))
                .map_err(|e| FsnError::internal(format!("age encryptor init failed: {e}")))?;
        ...
        .map_err(|e| FsnError::internal(format!("age armored writer failed: {e}")))?;

        let mut writer = encryptor
            .wrap_output(armored)
            .map_err(|e| FsnError::internal(format!("age wrap_output failed: {e}")))?;

        writer
            .write_all(plaintext)
            .map_err(|e| FsnError::internal(format!("age write failed: {e}")))?;

        writer
            .finish()
            .and_then(|w| w.finish())
            .map_err(|e| FsnError::internal(format!("age finish failed: {e}")))?;
...
impl AgeDecryptor {
    ...
    pub fn from_private_key(key: &str) -> Result<Self, FsnError> {
        let identity = key
            .parse::<age::x25519::Identity>()
            .map_err(|e| FsnError::config(format!("invalid age private key: {e}")))?;
        Ok(Self { identity })
    }

    /// Decrypt armored age ciphertext.
    pub fn decrypt(&self, ciphertext: &[u8]) -> Result<Vec<u8>, FsnError> {
        let armored = age::armor::ArmoredReader::new(ciphertext);
        let decryptor = age::Decryptor::new(armored)
            .map_err(|e| FsnError::internal(format!("age decryptor init failed: {e}")))?;

        let mut reader = decryptor
            .decrypt(std::iter::once(&self.identity as &dyn age::Identity))
            .map_err(|e| FsnError::internal(format!("age decrypt failed: {e}")))?;

        let mut plaintext = Vec::new();
        reader
            .read_to_end(&mut plaintext)
            .map_err(|e| FsnError::internal(format!("age read failed: {e}")))?;
...
impl AgePassphraseDecryptor {
    ...
    pub fn decrypt(&self, ciphertext: &[u8]) -> Result<Vec<u8>, FsnError> {
        ...
        let mut reader = decryptor
            ...
            .map_err(|e| FsnError::internal(format!("age passphrase decrypt failed: {e}")))?;
