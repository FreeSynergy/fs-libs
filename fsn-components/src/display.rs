// fsn-components/display.rs — Table, Progress, CodeBlock.
...
pub fn Table(props: TableProps) -> Element {
    let header_style = "background: var(--fsn-color-bg-sidebar, #0f172a); \
                        border-bottom: 1px solid var(--fsn-color-border-default, #334155);";

    rsx! {
        div {
            class: "fsn-table-wrap",
                    ...
                    for (row_idx, row) in props.rows.iter().enumerate() {
                        {
                            ...
                            rsx! {
                                tr {
                                    key: "{row_idx}",
                                    style: "border-bottom: 1px solid var(--fsn-color-border-default, #334155); \
...
pub fn Progress(props: ProgressProps) -> Element {
    ...
    let fill_color = match props.variant {
        BadgeVariant::Default => "var(--fsn-color-text-secondary, #94a3b8)",
        BadgeVariant::Success => "#22c55e",
        BadgeVariant::Warning => "#f59e0b",
        BadgeVariant::Error   => "#ef4444",
        BadgeVariant::Info    => "var(--fsn-color-primary, #06b6d4)",
    };

    rsx! {
        div {
            class: "fsn-progress-wrap",
...
pub fn CodeBlock(props: CodeBlockProps) -> Element {
    ...
    rsx! {
        div {
            class: "fsn-code-block",
            style: "position: relative; background: var(--fsn-color-bg-sidebar, #0f172a); \
