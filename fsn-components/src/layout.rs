// fsn-components/layout.rs — Tabs, Sidebar, StatusBar, Breadcrumb, SearchBar, ScrollContainer.
...
/// Single navigation item inside a `SidebarSection`.
#[derive(Clone, PartialEq)]
pub struct SidebarItem {
    ...
    pub icon: Option<String>,
    /// Optional badge label (e.g. a count).
...
/// A labelled group of `SidebarItem`s inside a `Sidebar`.
#[derive(Clone, PartialEq)]
...
impl SidebarSection {
    /// Construct a section with a title.
    pub fn new(title: impl Into<String>, items: Vec<SidebarItem>) -> Self {
        Self { title: Some(title.into()), items }
    }

    /// Construct an untitled section.
...
pub struct SidebarProps {
    ...
    pub active: String,
    /// Called with the newly selected item key.
    #[props(default)]
    pub on_change: EventHandler<String>,
    /// Optional CSS width. Defaults to `"220px"`.
    pub width: Option<String>,
}

/// Full sidebar navigation with sections, icons, and badge support.
///
/// ```no_run
/// Sidebar {
///     sections: vec![SidebarSection::new("Main", vec![SidebarItem::new("home", "Home")])],
///     active: active.read().clone(),
///     on_change: move |k| active.set(k),
/// }
/// ```
#[component]
pub fn Sidebar(props: SidebarProps) -> Element {
    let width = props.width.as_deref().unwrap_or("220px");

    rsx! {
        nav {
            class: "fsn-sidebar",
            aria_label: "Sidebar navigation",
            style: "width: {width}; min-height: 100%; flex-shrink: 0; \
            ...
            for section in &props.sections {
                div {
                    class: "fsn-sidebar-section",
                    style: "margin-bottom: 8px;",

                    if let Some(title) = &section.title {
                        div {
                            style: "padding: 6px 16px 4px; font-size: 10px; font-weight: 700; \
...
pub fn StatusBar(props: StatusBarProps) -> Element {
    rsx! {
        div {
            class: "fsn-status-bar",
            role: "status",
            style: "display: flex; align-items: center; justify-content: space-between; \
                    padding: 0 12px; height: 24px; flex-shrink: 0; \
                    background: var(--fsn-color-bg-sidebar, #0f172a); \
                    border-top: 1px solid var(--fsn-color-border-default, #334155); \
                    font-size: 11px; \
                    color: var(--fsn-color-text-muted, #64748b);",
            ...
            for (idx, item) in props.items.iter().enumerate() {
                {
                    ...
                    rsx! {
                        ...
                        if idx > 0 {
                            span {
                                aria_hidden: "true",
                                style: "color: var(--fsn-color-text-muted, #64748b);",
                                "›"
                            }
                        }
                        ...
                        if let Some(h) = href {
                            a {
                                href: "{h}",
                                style: "color: var(--fsn-color-text-secondary, #94a3b8); \
                            ...
                            span {
                                ...
                                style: "color: var(--fsn-color-text-primary, #e2e8f0);",
                                "{label}"
                            }
                        }
                    }
...
pub fn SearchBar(props: SearchBarProps) -> Element {
    ...
    let border_color = if *focused.read() {
        "var(--fsn-color-primary, #06b6d4)"
    } else {
        "var(--fsn-color-border-default, #334155)"
    };

    rsx! {
        div {
            class: "fsn-search-bar",
