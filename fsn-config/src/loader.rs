...
use fsn_error::{FsnError, Repairable, RepairOutcome};
...
impl ConfigLoader {
    ...
    pub fn load<T>(&self, path: &Path) -> Result<(T, Option<RepairOutcome>), FsnError>
    where
        T: DeserializeOwned + Repairable,
    {
        let full = self.resolve(path);
        let text = std::fs::read_to_string(&full)
            .map_err(|e| FsnError::Config(format!("cannot read {}: {}", full.display(), e)))?;

        let mut value: T = toml::from_str(&text)
            .map_err(|e| FsnError::Parse(format!("{}: {}", full.display(), e)))?;
    ...
    pub fn save<T: Serialize>(&self, path: &Path, value: &T) -> Result<(), FsnError> {
        let full = self.resolve(path);
        ...
        if let Some(parent) = full.parent() {
            std::fs::create_dir_all(parent).map_err(FsnError::Io)?;
        }

        let text = toml::to_string_pretty(value)
            .map_err(|e| FsnError::config(format!("TOML serialization failed: {e}")))?;

        // Write to a temp file first, then rename (atomic on most OS).
        let tmp = full.with_extension("toml.tmp");
        std::fs::write(&tmp, &text).map_err(FsnError::Io)?;
        std::fs::rename(&tmp, &full).map_err(FsnError::Io)?;

        Ok(())
    }

    /// Read raw TOML text from a file without deserializing or validating.
    pub fn read_raw(&self, path: &Path) -> Result<String, FsnError> {
        let full = self.resolve(path);
        std::fs::read_to_string(&full).map_err(FsnError::Io)
    }

    /// Write raw text to a file (no validation, with backup).
    pub fn write_raw(&self, path: &Path, content: &str) -> Result<(), FsnError> {
        let full = self.resolve(path);
        if full.exists() {
            self.backup(&full)?;
        }
        ...
        std::fs::write(&full, content).map_err(FsnError::Io)
    }

    /// Copy `path` → `path.bak` before overwriting.
    fn backup(&self, path: &Path) -> Result<(), FsnError> {
        let bak = path.with_extension("toml.bak");
        std::fs::copy(path, &bak).map_err(FsnError::Io)?;
        Ok(())
    }
}
...
impl FeatureFlags {
    ...
    pub fn load_json(path: &Path) -> Result<Self, FsnError> {
        if !path.exists() {
            return Ok(Self::default());
        }
        let text = std::fs::read_to_string(path)
            .map_err(|e| FsnError::Config(format!("cannot read flags {}: {e}", path.display())))?;
        serde_json::from_str(&text)
            .map_err(|e| FsnError::Parse(format!("feature flags JSON: {e}")))
    }

    /// Save flags to a JSON file.
    pub fn save_json(&self, path: &Path) -> Result<(), FsnError> {
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(FsnError::Io)?;
        }
        let text = serde_json::to_string_pretty(self)
            .map_err(|e| FsnError::config(format!("flags JSON serialization: {e}")))?;
        std::fs::write(path, text).map_err(FsnError::Io)
    }
...
pub fn parse_str<T: DeserializeOwned>(content: &str) -> Result<T, FsnError> {
    toml::from_str(content).map_err(|e| FsnError::Parse(format!("TOML parse error: {e}")))
}
...
pub fn load_toml<T: DeserializeOwned>(path: &Path) -> Result<T, FsnError> {
    let text = std::fs::read_to_string(path)
        .map_err(|e| FsnError::Config(format!("cannot read {}: {}", path.display(), e)))?;
    toml::from_str(&text)
        .map_err(|e| FsnError::Parse(format!("{}: {}", path.display(), e)))
}

/// Serialize and write a TOML file directly without a [`ConfigLoader`] instance.
pub fn save_toml<T: Serialize>(path: &Path, value: &T) -> Result<(), FsnError> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(FsnError::Io)?;
    }
    let text = toml::to_string_pretty(value)
...
mod tests {
    use super::*;
    use fsn_error::{RepairAction, RepairOutcome, ValidationIssue};
    ...
    fn save_and_load_roundtrip() {
        let dir = std::env::temp_dir().join("fsn-config-test");
    ...
    fn relative_path_resolved_from_base() {
        let dir = std::env::temp_dir().join("fsn-config-test-rel");
        std::fs::create_dir_all(&dir).unwrap();
        let loader = ConfigLoader::new(dir.clone());
    ...
    fn absolute_path_not_prefixed_with_base() {
        let dir   = std::env::temp_dir().join("fsn-config-test-abs");
        let other = std::env::temp_dir().join("fsn-config-test-abs-other");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn backup_created_on_overwrite() {
        let dir = std::env::temp_dir().join("fsn-config-test-bak");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn auto_repair_triggered_when_invalid() {
        let dir = std::env::temp_dir().join("fsn-config-test-repair");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn standalone_save_and_load_toml() {
        let path = std::env::temp_dir().join("fsn-config-standalone.toml");
    ...
    fn read_raw_returns_file_contents() {
        let dir = std::env::temp_dir().join("fsn-config-test-raw");
        std::fs::create_dir_all(&dir).unwrap();
