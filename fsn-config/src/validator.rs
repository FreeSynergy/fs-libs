...
use fsn_error::ValidationIssue;

use crate::schema::{ConfigSchema, FieldKind};
...
/// Check whether a TOML value matches the expected [`FieldKind`].
fn kind_matches(value: &Value, kind: &FieldKind) -> bool {
    match (value, kind) {
        (Value::String(_),  FieldKind::String)  => true,
        (Value::Integer(_), FieldKind::Integer) => true,
        (Value::Float(_),   FieldKind::Float)   => true,
        (Value::Boolean(_), FieldKind::Boolean) => true,
        (Value::Array(_),   FieldKind::Array)   => true,
        (Value::Table(_),   FieldKind::Table)   => true,
        _ => false,
    }
}
