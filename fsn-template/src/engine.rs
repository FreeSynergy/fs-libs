...
impl TemplateEngine {
    ...
    pub fn from_dir(dir: impl AsRef<std::path::Path>) -> Result<Self, FsnError> {
        let dir = dir.as_ref();

        if !dir.exists() {
            return Err(FsnError::not_found(format!(
        ...
        let mut tera = match tera::Tera::new(&tera_glob) {
            ...
            Err(e) => {
                ...
                if msg.contains("No match") || msg.contains("no match") || msg.contains("glob") {
                    ...
                    return Err(FsnError::internal(format!("template error: {e}")));
                }
            }
        };
        ...
        if let Err(e) = tera.add_template_files(
            glob::glob(&j2_glob)
                .map_err(|e| FsnError::internal(format!("glob error: {e}")))?
                .filter_map(|p| p.ok())
                .map(|p| (p, None::<String>))
                .collect::<Vec<_>>(),
        ) {
    ...
    pub fn add_template(
        ...
        self.tera
            ...
            .map_err(|e| FsnError::internal(format!("template error: {e}")))
    }

    /// Render a template string directly (not registered by name).
    pub fn render_str(&self, template: &str, ctx: &TemplateContext) -> Result<String, FsnError> {
    ...
    pub fn render(&self, name: &str, ctx: &TemplateContext) -> Result<String, FsnError> {
        self.tera
