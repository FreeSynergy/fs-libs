...
pub fn Window(props: WindowProps) -> Element {
    rsx! {
        div {
            class: "fsn-window {props.class}",
            role: "dialog",
            aria_label: "{props.title}",
            div {
                class: "fsn-window__titlebar",
                span { class: "fsn-window__title", "{props.title}" }
                if props.closable {
                    button {
                        class: "fsn-window__close",
                        aria_label: "Close",
                        onclick: move |e| {
            ...
            div { class: "fsn-window__body", {props.children} }
        }
    }
}
