...
pub fn ThemeSwitcher(props: ThemeSwitcherProps) -> Element {
    rsx! {
        div { class: "fsn-theme-switcher {props.class}", role: "group", aria_label: "Theme",
            for theme in &props.themes {
                {
                    let theme_name = theme.clone();
                    let is_active = *theme == props.current_theme;
                    let active_class = if is_active { "fsn-theme-switcher__btn--active" } else { "" };
                    rsx! {
                        button {
                            class: "fsn-theme-switcher__btn {active_class}",
