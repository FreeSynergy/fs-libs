...
impl QuadletManager {
    ...
    pub async fn create_quadlet(&self, service: &ServiceConfig) -> Result<PathBuf, FsnError> {
        ...
        if file_path.exists() {
            tokio::fs::copy(&file_path, &bak_path)
                .await
                .map_err(|e| FsnError::internal(format!("backup quadlet: {e}")))?;
        }

        let content = render_quadlet(service);
        tokio::fs::write(&file_path, content)
            .await
            .map_err(|e| FsnError::internal(format!("write quadlet {}: {e}", file_path.display())))?;
    ...
    pub async fn remove_quadlet(&self, name: &str) -> Result<(), FsnError> {
        let filename = format!("fsn-{}.container", name);
        let file_path = self.quadlet_dir.join(&filename);
        if file_path.exists() {
            tokio::fs::remove_file(&file_path)
                .await
                .map_err(|e| FsnError::internal(format!("remove quadlet {filename}: {e}")))?;
        }
        Ok(())
    }
    ...
    pub async fn rollback(&self, name: &str) -> Result<(), FsnError> {
        let filename     = format!("fsn-{}.container", name);
        let file_path    = self.quadlet_dir.join(&filename);
        let bak_path     = self.quadlet_dir.join(format!("{}.bak", filename));

        if !bak_path.exists() {
            return Err(FsnError::not_found(format!("no backup found for service {name}")));
        }

        tokio::fs::copy(&bak_path, &file_path)
            .await
            .map_err(|e| FsnError::internal(format!("restore quadlet from backup: {e}")))?;
    ...
    pub async fn reload_daemon(&self) -> Result<(), FsnError> {
        self.systemctl.daemon_reload().await
    }

    /// Run `systemctl --user start fsn-{name}.service`.
    pub async fn start_service(&self, name: &str) -> Result<(), FsnError> {
        self.systemctl.start(&unit(name)).await
    }

    /// Run `systemctl --user stop fsn-{name}.service`.
    pub async fn stop_service(&self, name: &str) -> Result<(), FsnError> {
        self.systemctl.stop(&unit(name)).await
    }

    /// Run `systemctl --user restart fsn-{name}.service`.
    pub async fn restart_service(&self, name: &str) -> Result<(), FsnError> {
        self.systemctl.restart(&unit(name)).await
    }

    /// Query the runtime status of `fsn-{name}.service`.
    pub async fn service_status(&self, name: &str) -> Result<ServiceStatus, FsnError> {
        self.systemctl.service_status(&unit(name)).await
    }

    /// Retrieve the last `lines` log lines for `fsn-{name}.service`.
    ///
    /// Runs `journalctl --user -u fsn-{name}.service -n {lines} --no-pager`.
    pub async fn service_logs(&self, name: &str, lines: usize) -> Result<Vec<String>, FsnError> {
        ...
        let output = tokio::process::Command::new("journalctl")
            ...
            .output()
            .await
            .map_err(|e| FsnError::internal(format!("journalctl subprocess: {e}")))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(FsnError::internal(format!("journalctl: {stderr}")));
        }
    ...
    async fn ensure_quadlet_dir(&self) -> Result<(), FsnError> {
        tokio::fs::create_dir_all(&self.quadlet_dir)
            .await
            .map_err(|e| FsnError::internal(format!("create quadlet dir: {e}")))
    }
}
...
fn render_quadlet(svc: &ServiceConfig) -> String {
    ...
    lines.push(format!("ContainerName=fsn-{}", svc.name));
...
/// Format `"fsn-{name}.service"`.
fn unit(name: &str) -> String {
    format!("fsn-{}.service", name)
}
...
mod tests {
    ...
    #[test]
    fn unit_name_format() {
        assert_eq!(unit("zentinel"), "fsn-zentinel.service");
    }
}
