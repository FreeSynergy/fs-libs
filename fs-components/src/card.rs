// fs-components/card.rs — Card, Badge, Divider, Spinner, Tooltip.
    ...
    rsx! {
        div {
            ...
            if let Some(label) = &props.label {
                span {
                    style: "font-size: 11px; color: var(--fs-color-text-muted, #64748b); \
...
pub fn Spinner(props: SpinnerProps) -> Element {
    ...
    let style = format!(
        "display: inline-block; width: {}px; height: {}px; border-radius: 50%; \
         border: {}px solid var(--fs-color-border-default, #334155); \
         border-top-color: var(--fs-color-primary, #06b6d4); \
...
pub fn Tooltip(props: TooltipProps) -> Element {
    rsx! {
        ...
        style {
            ".fs-tooltip:hover .fs-tooltip__bubble {{ opacity: 1; }}"
        }
    }
}
