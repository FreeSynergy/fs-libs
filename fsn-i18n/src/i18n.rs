...
impl I18n {
    ...
    pub fn load_dir(dir: &Path) -> Result<Self, FsnError> {
    ...
    pub fn add_ftl(&mut self, lang: &str, ftl_sources: &[String]) -> Result<(), FsnError> {
    ...
    pub fn add_toml_str(&mut self, lang: &str, toml_src: &str) -> Result<(), FsnError> {
        let value: toml::Value = toml_src
            .parse()
            .map_err(|e| FsnError::config(format!("invalid TOML for lang `{lang}`: {e}")))?;
...
fn collect_ftl_sources(dir: &Path) -> Result<Vec<String>, FsnError> {
    let mut sources = Vec::new();

    let entries = std::fs::read_dir(dir).map_err(|e| {
        FsnError::config(format!("cannot read directory `{}`: {e}", dir.display()))
    })?;
    ...
    for entry in entries.flatten() {
        ...
        let src = std::fs::read_to_string(&path).map_err(|e| {
            FsnError::config(format!("cannot read `{}`: {e}", path.display()))
        })?;
        sources.push(src);
    }
