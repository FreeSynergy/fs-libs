...
//! let label = fsn_i18n::t("action-save");
//! ```
...
pub fn init(dir: &Path) -> Result<(), FsnError> {
    let instance = I18n::load_dir(dir)?;
    GLOBAL_I18N
        .set(RwLock::new(instance))
        .map_err(|_| FsnError::internal("global I18n already initialized"))
}

/// Initialize the global [`I18n`] instance with an explicit active language.
///
/// Must be called before using [`t`] or [`t_with`].
pub fn init_with_lang(dir: &Path, lang: &str) -> Result<(), FsnError> {
...
///
/// Must be called before using [`t`] or [`t_with`].
pub fn init_with_builtins(active_lang: &str) -> Result<(), FsnError> {
...
///
/// Has no effect when the global instance has not been initialized yet.
pub fn add_toml_lang(lang: &str, toml_src: &str) -> Result<(), FsnError> {
    if let Some(lock) = GLOBAL_I18N.get() {
        if let Ok(mut i18n) = lock.write() {
...
///
/// Must be called before using [`t`] or [`t_with`].
pub fn init_with_toml_strs(active_lang: &str, locales: &[(&str, &str)]) -> Result<(), FsnError> {
