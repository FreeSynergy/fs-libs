...
pub fn Sidebar(props: SidebarProps) -> Element {
    rsx! {
        nav {
            class: "fs-sidebar {props.class}",
            aria_label: "Main navigation",
            ul { class: "fs-sidebar__list",
                for item in &props.items {
                    {
                        let item_id = item.id.clone();
                        let active_class = if item.active { "fs-sidebar__item--active" } else { "" };
                        rsx! {
                            li {
                                class: "fs-sidebar__item {active_class}",
                                button {
                                    class: "fs-sidebar__btn",
