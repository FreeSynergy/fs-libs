...
pub fn HelpPanel(props: HelpPanelProps) -> Element {
    rsx! {
        aside {
            class: "fs-help-panel {props.class}",
            aria_label: "Help panel",
            div { class: "fs-help-panel__header",
                h2 { class: "fs-help-panel__title", "{props.title}" }
                button {
                    class: "fs-help-panel__close",
            ...
            div { class: "fs-help-panel__body",
                p { class: "fs-help-panel__content", "{props.content}" }
            }
            if !props.related.is_empty() {
                div { class: "fs-help-panel__related",
                    h3 { class: "fs-help-panel__related-title", "Related topics" }
                    ul { class: "fs-help-panel__related-list",
                        for topic in &props.related {
                            li { class: "fs-help-panel__related-item",
                                span { class: "fs-help-panel__related-link", "{topic}" }
                            }
                        }
                    }
                }
