...
pub fn AppLauncher(props: AppLauncherProps) -> Element {
    rsx! {
        div { class: "fs-app-launcher {props.class}", role: "list",
            for app in &props.apps {
                {
                    let app_id = app.id.clone();
                    let status_class = format!("fs-app-launcher__status--{}", app.status);
                    rsx! {
                        div {
                            class: "fs-app-launcher__item",
                            role: "listitem",
                            button {
                                class: "fs-app-launcher__btn",
                                title: "{app.name}",
                                onclick: move |_| {
                                ...
                                span { class: "fs-app-launcher__icon", "{app.icon}" }
                                span { class: "fs-app-launcher__name", "{app.name}" }
                                span {
                                    class: "fs-app-launcher__status {status_class}",
                                    "{app.status}"
                                }
                            }
                        }
