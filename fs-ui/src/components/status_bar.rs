...
pub fn StatusBar(props: StatusBarProps) -> Element {
    rsx! {
        footer {
            class: "fs-status-bar {props.class}",
            role: "status",
            span { class: "fs-status-bar__left", "{props.left}" }
            if let Some(indicator) = &props.health_indicator {
                span { class: "fs-status-bar__health", aria_label: "Health", "{indicator}" }
            }
            span { class: "fs-status-bar__right", "{props.right}" }
        }
    }
}
