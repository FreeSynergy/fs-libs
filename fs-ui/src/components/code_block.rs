...
pub fn CodeBlock(props: CodeBlockProps) -> Element {
    rsx! {
        div { class: "fs-code-block {props.class}",
            if props.filename.is_some() || !props.language.is_empty() {
                div { class: "fs-code-block__header",
                    if let Some(filename) = &props.filename {
                        span { class: "fs-code-block__filename", "{filename}" }
                    }
                    if !props.language.is_empty() {
                        span { class: "fs-code-block__lang", "{props.language}" }
                    }
                }
            }
            pre { class: "fs-code-block__pre",
                code {
                    class: "fs-code-block__code language-{props.language}",
                    "{props.code}"
                }
            }
        }
