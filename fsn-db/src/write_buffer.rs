...
impl WriteBuffer {
    ...
    pub async fn enqueue(&self, write: BufferedWrite) -> Result<(), FsnError> {
    ...
    pub async fn flush(&self) -> Result<FlushResult, FsnError> {
        let mut queue = self.queue.lock().await;
        if queue.is_empty() {
        ...
        let txn = self
            ...
            .begin()
            .await
            .map_err(|e| FsnError::internal(format!("db transaction begin: {e}")))?;

        for write in writes {
            txn.execute_unprepared(&write.sql)
                .await
                .map_err(|e| FsnError::internal(format!("db execute: {e}")))?;
        }

        txn.commit()
            .await
            .map_err(|e| FsnError::internal(format!("db commit: {e}")))?;
