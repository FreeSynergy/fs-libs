...
impl Migrator {
    ...
    pub async fn run(db: &DatabaseConnection) -> Result<(), FsnError> {
    ...
    async fn ensure_tracking_table(db: &DatabaseConnection) -> Result<(), FsnError> {
        ...
        db.execute_unprepared(sql)
            .await
            .map_err(|e| FsnError::internal(format!("migration tracking table: {e}")))?;
        Ok(())
    }

    async fn is_applied(db: &DatabaseConnection, name: &str) -> Result<bool, FsnError> {
        ...
        let result = db
            ...
            .await
            .map_err(|e| FsnError::internal(format!("migration check: {e}")))?;
    ...
    async fn apply(db: &DatabaseConnection, name: &str, sql: &str) -> Result<(), FsnError> {
        // Execute each statement in the migration file individually.
        for stmt in sql.split(';').map(str::trim).filter(|s| !s.is_empty()) {
            db.execute_unprepared(stmt)
                .await
                .map_err(|e| FsnError::internal(format!("migration '{name}' failed: {e}")))?;
        }
        ...
        db.execute_unprepared(&record)
            .await
            .map_err(|e| FsnError::internal(format!("migration record '{name}': {e}")))?;
