...
pub type InstallHook = Box<dyn Fn(&InstallEvent) -> Result<(), FsnError> + Send + Sync>;
...
///
/// ```rust
/// use fsn_pkg::event::{EventBus, InstallEvent, InstallEventKind};
///
...
impl EventBus {
    ...
    pub fn emit(&self, event: &InstallEvent) -> Result<(), FsnError> {
        for (name, hook) in &self.hooks {
            hook(event).map_err(|e| {
                FsnError::internal(format!("install hook '{name}' failed: {e}"))
            })?;
        }
        Ok(())
    }
...
mod tests {
    ...
    fn failing_hook_aborts_dispatch() {
        ...
        let mut bus = EventBus::new();
        bus.register("fail", |_| Err(FsnError::internal("boom")));
