...
impl ApiManifest {
    ...
    pub fn from_toml(s: &str) -> Result<Self, FsnError> {
        toml::from_str(s)
            .map_err(|e| FsnError::parse(format!("manifest parse error: {e}")))
    }

    /// Parse a manifest from a file.
    pub fn from_file(path: &Path) -> Result<Self, FsnError> {
        let s = std::fs::read_to_string(path)
            .map_err(|e| FsnError::internal(format!("cannot read {}: {e}", path.display())))?;
        Self::from_toml(&s)
    }

    /// Serialize to TOML.
    pub fn to_toml(&self) -> Result<String, FsnError> {
        toml::to_string_pretty(self)
            .map_err(|e| FsnError::internal(format!("manifest serialize error: {e}")))
    }
}
...
impl PackageSource {
    ...
    pub fn oci_ref(&self) -> Option<Result<OciRef, FsnError>> {
        match self {
