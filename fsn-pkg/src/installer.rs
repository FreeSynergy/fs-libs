...
///
/// ```rust,ignore
/// use fsn_pkg::{ApiManifest, InstallOptions, PackageInstaller};
///
...
impl PackageInstaller {
    ...
    pub fn register_hook(
        ...
        hook: impl Fn(&InstallEvent) -> Result<(), FsnError> + Send + Sync + 'static,
    ) {
    ...
    pub fn install(
    ...
    ) -> Result<InstallOutcome, FsnError> {
        ...
        for (src, dest_template) in all_files {
            let dest = expand_vars(dest_template, &options.vars);
            let dest_path = PathBuf::from(&dest);

            if !options.dry_run {
                if let Some(parent) = dest_path.parent() {
                    fs::create_dir_all(parent).map_err(|e| {
                        FsnError::internal(format!(
...
fn run_hook(cmd: &str, vars: &HashMap<String, String>, dry_run: bool) -> Result<(), FsnError> {
    let expanded = expand_vars(cmd, vars);
    if dry_run {
    ...
    let status = Command::new("sh")
        ...
        .arg(&expanded)
        .status()
        .map_err(|e| FsnError::internal(format!("hook spawn failed `{expanded}`: {e}")))?;
    if !status.success() {
...
"#;
    ...
    #[test]
    fn expand_vars_substitutes_placeholders() {
        let mut vars = HashMap::new();
        vars.insert("data_root".into(), "/srv/data".into());
        vars.insert("name".into(), "zentinel".into());

        let result = super::expand_vars("{data_root}/{name}/config.toml", &vars);
