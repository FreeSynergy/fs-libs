...
impl OciRef {
    ...
    pub fn parse(s: &str) -> Result<Self, FsnError> {
        if s.is_empty() {
            return Err(FsnError::parse("OCI reference must not be empty"));
        }
        ...
        if repository.is_empty() {
            return Err(FsnError::parse("OCI reference: repository must not be empty"));
        }
...
fn split_tag(s: &str) -> Result<(&str, Option<String>), FsnError> {
...
fn validate_tag(tag: &str) -> Result<(), FsnError> {
    if tag.is_empty() || tag.len() > 128 {
        return Err(FsnError::parse(format!(
...
fn validate_digest(digest: &str) -> Result<(), FsnError> {
    if !digest.contains(':') {
        return Err(FsnError::parse(format!(
