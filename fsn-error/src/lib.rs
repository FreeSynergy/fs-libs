// fsn-error — Error handling + Repairable trait for the FreeSynergy ecosystem.
//
// Modules:
//   validation  — IssueSeverity, ValidationIssue
//   repair      — RepairAction, RepairOption, RepairOutcome, Repairable trait
//
// FsnError lives here in lib.rs (main error enum, covers all subsystems).
...
///
/// Prefer `FsnError` in new code.
pub type FsyError = FsnError;
...
mod tests {
    ...
    #[test]
    fn fsn_error_display() {
        let e = FsnError::config("missing field");
        assert!(e.to_string().contains("missing field"));
    }

    #[test]
    fn fsn_error_all_constructors() {
        assert!(FsnError::parse("x").to_string().contains("x"));
        assert!(FsnError::not_found("x").to_string().contains("x"));
        assert!(FsnError::network("x").to_string().contains("x"));
        assert!(FsnError::internal("x").to_string().contains("x"));
        assert!(FsnError::auth("x").to_string().contains("x"));
        let e = FsnError::validation("field", "bad");
