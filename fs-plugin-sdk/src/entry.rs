...
///     fs_plugin_sdk::run_plugin(&router);
/// }
/// ```
pub fn run_plugin(router: &CommandRouter) {
    ...
    if let Err(e) = io::stdin().read_to_string(&mut input) {
        eprintln!("fs-plugin-sdk: failed to read stdin: {e}");
        std::process::exit(1);
    }
    ...
    let ctx: PluginContext = match serde_json::from_str(&input) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("fs-plugin-sdk: invalid PluginContext JSON: {e}");
            std::process::exit(1);
        }
    };
    ...
    if ctx.protocol != 1 {
        eprintln!(
            "fs-plugin-sdk: unsupported protocol version {} (expected 1)",
            ctx.protocol
        );
        std::process::exit(1);
    }
    ...
    let output = match serde_json::to_string(&response) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("fs-plugin-sdk: failed to serialize PluginResponse: {e}");
            std::process::exit(1);
        }
    };

    if let Err(e) = io::stdout().write_all(output.as_bytes()) {
        eprintln!("fs-plugin-sdk: failed to write stdout: {e}");
        std::process::exit(1);
    }
}
