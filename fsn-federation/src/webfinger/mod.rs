// fsn-federation/src/webfinger/mod.rs — WebFinger (RFC 7033) client + types
...
impl WebFingerClient {
    ...
    pub async fn lookup(
        ...
        host: &str,
        resource: &str,
    ) -> Result<WebFingerResponse, FsnError> {
        let url = format!(
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| FsnError::network(format!("WebFinger request failed: {e}")))?;
        ...
        resp.json::<WebFingerResponse>()
            .await
            .map_err(|e| FsnError::parse(format!("WebFinger JSON parse failed: {e}")))
    }
