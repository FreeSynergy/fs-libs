...
impl StoreSource {
    ...
    async fn fetch_text_once(&self, relative_path: &str) -> Result<String, FsnError> {
        match self {
            Self::Local(root) => {
                let path = root.join(relative_path);
                debug!("store read local: {}", path.display());
                tokio::fs::read_to_string(&path).await.map_err(|e| {
                    FsnError::internal(format!("store local read '{}': {e}", path.display()))
                })
            }
            Self::Http(base) => {
                ...
                let client = reqwest::Client::builder()
                    ...
                    .build()
                    .map_err(|e| FsnError::internal(format!("reqwest build: {e}")))?;
                let resp = client.get(&url).send().await.map_err(|e| {
                    FsnError::network(format!("store http fetch '{url}': {e}"))
                })?;
                ...
                resp.text().await.map_err(|e| {
                    FsnError::network(format!("store http read body '{url}': {e}"))
                })
            }
        }
    }
...
impl StoreClient {
    ...
    pub async fn fetch_catalog<M>(&mut self, namespace: &str, force: bool) -> Result<Catalog<M>, FsnError>
    where
    ...
    pub async fn fetch_i18n(&self, namespace: &str, locale_code: &str) -> Result<I18nBundle, FsnError> {
        ...
        let meta: I18nManifestWrapper = toml::from_str(&meta_text)
            .map_err(|e| FsnError::parse(format!("i18n manifest parse: {e}")))?;

        let ui: toml::Value = toml::from_str(&ui_text)
            .map_err(|e| FsnError::parse(format!("ui.toml parse: {e}")))?;
    ...
    pub async fn fetch_raw(&self, relative_path: &str) -> Result<String, FsnError> {
        self.source
