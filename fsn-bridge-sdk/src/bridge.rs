...
pub trait Bridge: Send + Sync {
    ...
    fn probe(&self) -> impl std::future::Future<Output = Result<BridgeInfo, FsnError>> + Send;
}
...
pub trait ProbableBridge: Send + Sync {
    ...
    fn probe(
        &self,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<BridgeInfo, FsnError>> + Send + '_>>;
}
