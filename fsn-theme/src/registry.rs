// fsn-theme — ThemeRegistry: manage and switch between multiple themes.
...
/// # Example
/// ```
/// use fsn_theme::{ThemeRegistry, Theme};
///
...
impl ThemeRegistry {
    ...
    pub fn register_toml_str(&mut self, toml_str: &str) -> Result<&Theme, FsnError> {
    ...
    pub fn register_css(&mut self, css: &str, name: &str) -> Result<&Theme, FsnError> {
    ...
    pub fn set_active(&mut self, name: &str) -> Result<(), FsnError> {
        if self.themes.contains_key(name) {
            self.active = name.to_string();
            Ok(())
        } else {
            Err(FsnError::Config(format!("theme '{name}' not registered")))
        }
    }
    ...
    pub fn remove(&mut self, name: &str) -> Result<(), FsnError> {
        if self.active == name {
            return Err(FsnError::Config(format!("cannot remove active theme '{name}'")));
        }
