...
//! Join-token generation and verification for `FreeSynergy` node federation.
//!
//! A join token allows a new node to securely join an existing FS cluster.
...
impl fmt::Display for JoinTokenError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Expired => write!(f, "join token has expired"),
            Self::AddressMismatch => write!(f, "join token address mismatch"),
            Self::InvalidFormat => write!(f, "join token has invalid format"),
        }
    }
}
...
/// A join token for FS cluster federation.
///
...
impl JoinToken {
    ...
    pub fn parse(token_str: &str) -> Result<Self, JoinTokenError> {
        ...
        let decoded = base64url::decode(payload).map_err(|()| JoinTokenError::InvalidFormat)?;
    ...
    ///
    /// # Errors
    /// Returns `JoinTokenError::Expired` when past its TTL, or
    /// `JoinTokenError::AddressMismatch` when the address differs.
...
impl fmt::Display for JoinToken {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(f, "fsn1.{}", base64url::encode(payload.as_bytes()))
    }
}
