...
impl ProcessPluginRunner {
    ...
    pub fn run(&self, ctx: &PluginContext) -> Result<PluginResponse, FsnError> {
        ...
        // Write context JSON to stdin; drop handle to close the pipe (EOF for plugin)
        {
            ...
            stdin.write_all(ctx_json.as_bytes()).map_err(|e| {
                FsnError::Plugin(format!("failed to write to plugin stdin: {e}"))
            })?;
        }

        let output = child.wait_with_output().map_err(|e| {
            FsnError::Plugin(format!("plugin process error: {e}"))
        })?;
        ...
        let response: PluginResponse =
            serde_json::from_slice(&output.stdout).map_err(|e| {
                FsnError::Plugin(format!("invalid JSON from plugin stdout: {e}"))
            })?;
        ...
        if response.has_error() {
            return Err(FsnError::Plugin(format!("plugin error: {}", response.error)));
        }
    ...
    fn write_files(&self, response: &PluginResponse) -> Result<(), FsnError> {
        for file in &response.files {
            ...
            fs::write(dest, &file.content).map_err(|e| {
                FsnError::Plugin(format!("failed to write {}: {e}", dest.display()))
            })?;
