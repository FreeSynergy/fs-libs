...
use fsn_plugin_sdk::PluginManifest;
...
impl PluginRuntime {
    ...
    pub fn new() -> Result<Self, FsnError> {
        let engine = Engine::default();
        Ok(Self { engine })
    }

    /// Load a plugin from WASM bytes with the given sandbox.
    pub fn load(&self, wasm_bytes: &[u8], sandbox: PluginSandbox) -> Result<PluginHandle, FsnError> {
        let module = Module::new(&self.engine, wasm_bytes)
            .map_err(|e| FsnError::Plugin(format!("failed to compile WASM module: {e}")))?;
        self.instantiate(module, sandbox)
    }

    /// Load a plugin from a WASM file with the given sandbox.
    pub fn load_file(&self, path: &Path, sandbox: PluginSandbox) -> Result<PluginHandle, FsnError> {
        let module = Module::from_file(&self.engine, path).map_err(|e| {
            FsnError::Plugin(format!("failed to load WASM file {}: {e}", path.display()))
        })?;
        self.instantiate(module, sandbox)
    }
    ...
    fn instantiate(&self, module: Module, sandbox: PluginSandbox) -> Result<PluginHandle, FsnError> {
        ...
        wasmtime_wasi::preview1::add_to_linker_sync(&mut linker, |cx: &mut WasiP1Ctx| cx)
            .map_err(|e| FsnError::Plugin(format!("failed to add WASI to linker: {e}")))?;

        let instance = linker
            .instantiate(&mut store, &module)
            .map_err(|e| FsnError::Plugin(format!("failed to instantiate WASM module: {e}")))?;
...
fn read_manifest(
    ...
    instance: &Instance,
) -> Result<PluginManifest, FsnError> {
    let manifest_fn = instance
        .get_func(&mut *store, "plugin_manifest_json")
        .ok_or_else(|| FsnError::Plugin("WASM export `plugin_manifest_json` not found".into()))?;

    let mut results = vec![Val::I32(0)];
    manifest_fn
        .call(&mut *store, &[], &mut results)
        .map_err(|e| FsnError::Plugin(format!("plugin_manifest_json call failed: {e}")))?;

    let ptr = results[0]
        .i32()
        .ok_or_else(|| FsnError::Plugin("plugin_manifest_json returned non-i32".into()))?
        as usize;

    let memory = instance
        .get_memory(&mut *store, "memory")
        .ok_or_else(|| FsnError::Plugin("WASM export `memory` not found".into()))?;
    ...
    let end = mem_data[ptr..]
        .iter()
        .position(|&b| b == 0)
        .ok_or_else(|| FsnError::Plugin("manifest JSON missing null terminator".into()))?;

    let json_bytes = mem_data[ptr..ptr + end].to_vec();
    let json_str = String::from_utf8(json_bytes)
        .map_err(|e| FsnError::Plugin(format!("manifest JSON is not valid UTF-8: {e}")))?;

    let manifest: PluginManifest = serde_json::from_str(&json_str)
        .map_err(|e| FsnError::Plugin(format!("failed to parse plugin manifest: {e}")))?;
