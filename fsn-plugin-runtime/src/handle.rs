...
use fsn_plugin_sdk::{PluginContext, PluginManifest, PluginResponse};
...
impl PluginHandle {
    ...
    pub fn execute(&mut self, input: &PluginContext) -> Result<PluginResponse, FsnError> {
        let command = input.command.clone();
        let context_json = serde_json::to_string(input).map_err(|e| {
            FsnError::Plugin(format!("failed to serialize PluginContext: {e}"))
        })?;
        ...
        let execute_fn = instance
            ...
            .ok_or_else(|| FsnError::Plugin("WASM export `plugin_execute` not found".into()))?;
        ...
        execute_fn
            ...
            .map_err(|e| FsnError::Plugin(format!("plugin_execute call failed: {e}")))?;

        let result_ptr = results[0]
            .i32()
            .ok_or_else(|| FsnError::Plugin("plugin_execute returned non-i32".into()))?
            as usize;
        ...
        let response: PluginResponse = serde_json::from_str(&json_str).map_err(|e| {
            FsnError::Plugin(format!("invalid JSON from plugin_execute: {e}"))
        })?;

        if response.has_error() {
            return Err(FsnError::Plugin(format!(
                "plugin '{}' error: {}",
...
fn wasm_alloc(store: &mut Store<WasiP1Ctx>, instance: &Instance, len: usize) -> Result<usize, FsnError> {
    let alloc_fn = instance
        .get_func(&mut *store, "plugin_alloc")
        .ok_or_else(|| FsnError::Plugin("WASM export `plugin_alloc` not found".into()))?;

    let mut results = vec![Val::I32(0)];
    alloc_fn
        .call(&mut *store, &[Val::I32(len as i32)], &mut results)
        .map_err(|e| FsnError::Plugin(format!("plugin_alloc call failed: {e}")))?;

    let ptr = results[0]
        .i32()
        .ok_or_else(|| FsnError::Plugin("plugin_alloc returned non-i32".into()))?
        as usize;
...
fn wasm_write(
    ...
    memory
        ...
        .map_err(|e| FsnError::Plugin(format!("WASM memory write failed: {e}")))?;
...
fn wasm_read_cstr(
    ...
    let end = mem_data[offset..]
        .iter()
        .position(|&b| b == 0)
        .ok_or_else(|| FsnError::Plugin("WASM string missing null terminator".into()))?;

    let bytes = mem_data[offset..offset + end].to_vec();
    String::from_utf8(bytes)
        .map_err(|e| FsnError::Plugin(format!("WASM string is not valid UTF-8: {e}")))
}
